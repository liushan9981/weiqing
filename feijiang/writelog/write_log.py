...
def mylog(when, file):
    ...
    def pre_logged(func_or_method):
        def wrapper(*args, **kwargs):
            log(func_or_method, *args, **kwargs)
            return func_or_method(*args, **kwargs)
        return wrapper

    def post_logged(func_or_method):
        if isinstance(func_or_method, types.FunctionType):
            def wrapper_func(*args, **kwargs):
                try:
                    return func_or_method(*args, **kwargs)
                finally:
                    log(func_or_method.__name__)
            return wrapper_func
        else:
            class_name = func_or_method.__name__
            orig_attr = func_or_method.__getattribute__

            def wrapper_cls(self, method_name, *args, **kwargs):
                instance_method_name = "%s().%s" % (class_name, method_name)
                try:
                    return orig_attr(self, method_name, *args, **kwargs)
                finally:
                    log(instance_method_name)
            func_or_method.__getattribute__ = wrapper_cls
